"""
ChatGPT 批量自动注册工具 (并发版) - 支持 DuckMail / TempMail.lol / CF 自建邮箱 / YYDS Mail
...
YYDSMAIL_API_BASE = _CONFIG.get("yydsmail_api_base", "https://maliapi.215.im/v1").rstrip("/")
YYDSMAIL_API_KEY = _CONFIG.get("yydsmail_api_key", "")
...
def _quick_preflight(proxy: str = None, provider: str = "duckmail") -> bool:
    ...
    # 5) YYDS Mail 可达性（仅 yydsmail 时检查）
    if provider == "yydsmail":
        try:
            headers = {}
            if YYDSMAIL_API_KEY:
                headers["X-API-Key"] = YYDSMAIL_API_KEY
            r = sess.get(f"{YYDSMAIL_API_BASE}/domains", headers=headers, timeout=20, allow_redirects=True)
            ok = r.status_code < 500
            # 额外检查：确认有可用的已验证域名
            domain_count = 0
            if ok and r.status_code == 200:
                try:
                    ddata = r.json()
                    domains_list = ddata.get("data") or ddata.get("domains") or (ddata if isinstance(ddata, list) else [])
                    domain_count = len([d for d in domains_list if isinstance(d, dict) and d.get("isVerified")])
                except Exception:
                    pass
            if domain_count == 0:
                ok = False
            _record("yydsmail api", ok, f"status={r.status_code}, 可用域名={domain_count}")
        except Exception as e:
            _record("yydsmail api", False, f"异常: {e}")
...
class ChatGPTRegister:
    ...
    def _fetch_yydsmail_domains(self):
        """获取 YYDS Mail 可用的已验证域名列表"""
        ...
        try:
            ...
            if resp.status_code != 200:
                self._print(f"[yydsmail] 获取域名列表失败: {resp.status_code}")
                return []
            data = resp.json() if resp.content else {}
            domains_list = data.get("data") or data.get("domains") or (data if isinstance(data, list) else [])
            # 过滤出已验证的域名
            verified = [d["domain"] for d in domains_list
                        if isinstance(d, dict) and d.get("isVerified") and d.get("domain")]
            return verified
        except Exception as e:
            self._print(f"[yydsmail] 获取域名列表异常: {e}")
            return []

    def create_yydsmail_email(self):
        """创建 YYDS Mail 临时邮箱，返回 (email, password, mail_token)"""
        api_base = YYDSMAIL_API_BASE.rstrip("/")
        headers = {"Content-Type": "application/json"}
        if YYDSMAIL_API_KEY:
            headers["X-API-Key"] = YYDSMAIL_API_KEY
        ...
        # 获取可用域名并随机选择一个
        available_domains = self._fetch_yydsmail_domains()
        if not available_domains:
            raise Exception("YYDS Mail 无可用的已验证域名，请检查 API Key 权限或稍后重试")
        chosen_domain = random.choice(available_domains)
        self._print(f"[yydsmail] 选择域名: {chosen_domain}")

        try:
            resp = curl_requests.post(
                f"{api_base}/accounts",
                json={"domain": chosen_domain},
                headers=headers,
                proxies=proxies,
                timeout=15,
                impersonate=self.impersonate,
            ...
            raise Exception(f"YYDS Mail 请求异常: {e}")

        if resp.status_code not in (200, 201):
            raise Exception(f"YYDS Mail 创建失败: {resp.status_code} - {resp.text[:200]}")

        data = resp.json() if resp.content else {}
        # API 返回 {"success": true, "data": {...}} 信封格式，需解包
        if isinstance(data, dict) and "data" in data and isinstance(data["data"], dict):
            data = data["data"]
        email = str(data.get("address") or data.get("email") or "").strip()
        token = str(data.get("token") or "").strip()
        if not email or not token:
            raise Exception(f"YYDS Mail 返回数据不完整（address/email 或 token 为空），raw={str(data)[:200]}")

        self._yydsmail_address = email
        self._print(f"[yydsmail] 创建邮箱成功: {email}")
        return email, "", token

    def _fetch_emails_yydsmail(self, mail_token: str):
        """从 YYDS Mail 拉取邮件列表"""
        api_base = YYDSMAIL_API_BASE.rstrip("/")
        headers = {}
        if YYDSMAIL_API_KEY:
        ...
        proxies = {"http": self.proxy, "https": self.proxy} if self.proxy else None
        try:
            address = getattr(self, "_yydsmail_address", "")
            ...
            if resp.status_code != 200:
                return []
            data = resp.json() if resp.content else {}
            # API 返回 {"success": true, "data": {"messages": [...], "total": N}} 信封格式
            if isinstance(data, dict) and "data" in data:
                inner = data["data"]
                if isinstance(inner, list):
                    return inner
                if isinstance(inner, dict):
                    return inner.get("messages") or inner.get("items") or []
            if isinstance(data, list):
                return data
            return data.get("messages") or data.get("data") or data.get("items") or []
        except Exception:
            return []

    def _fetch_email_detail_yydsmail(self, mail_token: str, msg_id: str):
        """获取 YYDS Mail 单封邮件详情"""
        api_base = YYDSMAIL_API_BASE.rstrip("/")
        headers = {}
        if YYDSMAIL_API_KEY:
    ...
    def _extract_yydsmail_code(self, messages: list) -> Optional[str]:
        """从 YYDS Mail 邮件列表中提取验证码"""
        for msg in messages:
            if not isinstance(msg, dict):
                continue
            content = " ".join([
                str(msg.get("subject") or ""),
                str(msg.get("body") or msg.get("text") or ""),
                str(msg.get("html") or ""),
                str(msg.get("from") or ""),
            ])
            if "openai" not in content.lower() and "chatgpt" not in content.lower():
                # 邮件列表可能不含正文，尝试通过 subject/from 过滤
                subject = str(msg.get("subject") or "").lower()
                sender = str(msg.get("from") or "").lower()
                if "openai" not in sender and "chatgpt" not in subject:
                    continue
            code = self._extract_verification_code(content)
            if code:
                return code
    ...
    def wait_for_verification_email(self, mail_token: str, timeout: int = 120,
        ...
        while time.time() - start_time < timeout:
            ...
            if effective_provider == "cfmail":
            ...
            elif effective_provider == "yydsmail":
                ...
                for msg in messages:
                    ...
                    if msg_id and msg_id not in seen_ids:
                        ...
                        new_messages.append(msg)
                if new_messages:
                    code = self._extract_yydsmail_code(new_messages)
                    if not code:
                        # 列表可能无正文，逐封获取详情
                        for msg in new_messages:
                            detail = self._fetch_email_detail_yydsmail(mail_token, str(msg.get("id", "")))
                            if detail:
                                body = str(detail.get("text") or detail.get("html") or detail.get("body") or "")
            ...
            while time.time() < otp_deadline and not otp_success:
                ...
                if provider == "cfmail":
                    ...
                    for msg in messages[:20]:
                        ...
                        if not msg_id:
                            continue
                        detail = self._fetch_email_detail_yydsmail(mail_token, str(msg_id))
                        if not detail:
                            continue
                        content = " ".join([
...
def _register_one(idx, total, proxy, output_file):
    ...
    try:
        ...
        if provider == "cfmail":
            ...
            reg._print("[yydsmail] 创建 YYDS Mail 临时邮箱...")
            email, email_pwd, mail_token = reg.create_yydsmail_email()
        else:
            reg._print("[DuckMail] 创建临时邮箱...")
            if not DUCKMAIL_BEARER:
                raise Exception("DUCKMAIL_BEARER 未设置，请在 config.json 中配置或改用 cfmail/tempmail_lol/yydsmail")
...
def run_batch(total_accounts: int = 3, output_file="registered_accounts.txt",
    ...
    if provider == "cfmail":
        ...
        if not DUCKMAIL_BEARER:
            ...
            print("   请在 config.json 中设置 duckmail_bearer，或将 mail_provider 改为 cfmail/tempmail_lol/yydsmail")
            return
    elif provider == "yydsmail":
        if not YYDSMAIL_API_KEY:
            print("❌ 错误: mail_provider=yydsmail 但未设置 YYDSMAIL_API_KEY")
            print("   请在 config.json 中设置 yydsmail_api_key")
            return
    elif provider != "tempmail_lol":
        print(f"❌ 错误: 不支持的 mail_provider={provider}")
        print("   可选值: duckmail / cfmail / tempmail_lol / yydsmail")
        return
...
def main():
    ...
    if provider == "cfmail":
        ...
        if not DUCKMAIL_BEARER:
            ...
            print("   请编辑 config.json 设置 duckmail_bearer，或将 mail_provider 改为 cfmail/tempmail_lol/yydsmail")
            print("\n   按 Enter 继续尝试运行 (可能会失败)...")
            input()
    elif provider == "tempmail_lol":
        ...
        if not YYDSMAIL_API_KEY:
            ...
            print("   请编辑 config.json 设置 yydsmail_api_key")
            print("\n   按 Enter 继续尝试运行 (可能会失败)...")
            input()
        else:
            print(f"\n[Info] YYDS Mail 已启用: {YYDSMAIL_API_BASE}")
    else:
